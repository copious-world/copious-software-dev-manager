...
class BundleOps {
    ...
    async ensure_compressed_bundles_exists(bundle_storage_directory,bundle_compressed_directory) {
        // runs sync
        this.sys_coms.bash_command("./tools/run_compression.sh",`${bundle_storage_directory} ${bundle_compressed_directory}`)
    }
    ...
    async internal_publish(params,concerns_file_map) {
        ...
        for ( let config_file in concerns_file_map ) {
            ...
            for ( let concern in concerns_table ) {
                ...
                if ( params.compress ) {
                    bundle_storage_directory = await this.ensure_compressed_bundles_exists(bundle_storage_directory,bundle_compressed_directory)
                    reports.push(`testing cp -r ${bundle_storage_directory} ${dest}/`)
                    //this.sys_coms.spawn_command('cp',['-RT', bundle_storage_directory, `${dest}/${params.rename_compressed}`])
                } else {
                    reports.push(`cp -r ${bundle_storage_directory} ${dest}/`)
                    this.sys_coms.spawn_command('cp',['-r', bundle_storage_directory, `${dest}/`])
                }
            }
        }
        OK = true
