...
class KanbanOps {
    ...
    constructor(conf) {
        ...
        this.kb_map_fields = ["kanbans", "kb_maps", "kb_infos"]
    }


    merge_kanbans(update,all_kb_map) {
        //
        for ( let field of this.kb_map_fields ) {
            let mapped_data = update[field]
            let current_mapped_data = all_kb_map[field]
            for ( let name in mapped_data ) {
                current_mapped_data[name] = mapped_data[name]
            }
        }
        //
        return all_kb_map
    ...
    async unload_kanban(params) {
        ...
        switch ( params.scope ){
            case "all-boards" : {
                ...
                if ( !(all_kbs) && (Object.keys(this.all_kb_map.kanbans).length > 0) ) {
                    all_kbs = this.all_kb_map.kanbans
                }
                if ( !(all_kbm) && (Object.keys(this.all_kb_map.kb_maps).length > 0) ) {
                    all_kbm = this.all_kb_map.kb_maps
                }
                if ( !(all_kbi) && (Object.keys(this.all_kb_map.kb_infos).length > 0) ) {
                ...
                let merge_update = this.merge_kanbans(update,this.all_kb_map)

                this.all_kb_map = merge_update
    ...
    get_kanban_as_string(title) {
        if ( title !== undefined && title !== "all-boards") {
            //
console.log(title)
            //
            try {
