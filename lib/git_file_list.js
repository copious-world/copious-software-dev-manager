        ...
        try {
            ...
            switch ( cmd_str ) {
                ...
                case "last-commit-date" : {
                    let date_data = await this.git.log({"-1" : null,"--format" : "%cd"})
                    let date = new Date(date_data.latest.hash)
    ...
    async list_git_directories(top_level_dir,git_list) {
        ...
        for ( let file of dir_list ) {
            ...
            if ( fs.existsSync(`${top_level_dir}/.git`) ) {
                ...
                //
                let revision_date = await this.git_command_line(top_level_dir,"last-commit-date")
console.log(revision_date,top_level_dir)
                //
                let dir_info = {
                    "name" : `${top_level_dir}`,
                    "is_empty" : empty_git,
