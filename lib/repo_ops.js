...
class RepoOps {
    ...
    constructor(conf,all_p,sys_ops) {
    ...
    check_admin_pass(password) {
        if ( this._conf.password === undefined ) {
            return true
        }
...
//                 }
    ...
    async most_recent_change(git_dir,modified) {
        let change_times = []
        for ( let mod of modified ) {
            let stat = fs.statSync(`${git_dir}/${mod}`)
            if ( stat ) {
                change_times.push(stat.mtimeMs)
            }
        }
        return Math.max(...change_times)
    }
...
//console.dir(file_info)
                    ...
                    file_list.ahead = file_info.ahead
                    file_list.behind = file_info.behind
                    file_list.clean = file_info.isClean()
                    return file_list
                } catch (e) {
console.log(e)
                }
            ...
            case "get-changes" : {
                ...
                for ( let repo of rlist ) {
                    if ( repo.in_sync ) {
                        ...
                        try {
                            await this.git.cwd({ path: git_dir, root: true });
                            let file_info = await this.git.status({"--porcelain" : "v1"})
                            if ( file_info.modified.length ) {

                                let last_mod = await this.most_recent_change(git_dir,file_info.modified)
                                
                                change_log[git_dir] = {
                                    "changes" : file_info.modified,
                                    "recency" : (now - last_mod)
                                }
                            }
                        } catch (e) {
                            console.log(e)
...
//                break;
            ...
            case "commit" : {
                ...
                try {
                    await this.git.cwd({ path: git_dir, root: true });
                    let file_info = await this.git.status()
                    if ( Array.isArray(file_info.not_added) && (file_info.not_added.length > 0 ) ) {
                        await this.git.add(file_info.not_added)
                    }
                    if ( Array.isArray(file_info.modified) && (file_info.modified.length > 0 ) ) {
                        await this.git.add(file_info.modified)
                    }

                    let messages = params.messages
                    if ( !messages || messages.length === 9 ) {
                        messages = ["update for backup purposes","this message given in the absence of other comments"]
                    }
                    await this.git.commit(messages)
...
console.log(e)
            ...
            case "publish-npm" : {
                ...
                let git_dir = params.parameters
                try {
                    const stdout = execFileSync('bash', ['../bash/npm-releaser.sh', git_dir], {
...
*/
    ...
    // lsof -a -d cwd -c bash   -- terminals working directories
    ...
    async data_read_op(obj) {
        ...
        let admin_OK = this.check_admin_pass(admin_pass)
        if ( admin_OK ) {
            //
    ...
    // gnome-terminal --tab --working-directory="/path/to/folder"
...
console.dir(obj,{ depth : 4 })
            ...
            if ( operation ) {
                switch ( operation.name ) {
                    ...
                    case "exec" : {
                        ...
                        if ( typeof obj.sources === "string" ) {
                            if ( cmd_obj.args.indexOf(obj.sources) >= 0 ) {
                                let srcstr = obj.sources.trim()
                                if ( srcstr[0] === '[' ) {
                                    let asrcstr = this.absolute_path(srcstr)
                                    asrcstr += '/'
                                    cmd_obj.args = cmd_obj.args.replace(srcstr,asrcstr)
                                }
                            }
                        }
                        ...
                        let p = new Promise((resolve,reject) => {
                            exec(cmd_list, (error, stdout, stderr) => {
                                ...
                                let html_fix = `<div><pre>${stdout}</pre></div>`
    ...
    // async test() {
    //     let cmd_str = "last-commit-date" 
    //     let git_dir = "/home/richard/GitHub/petri-net/run-petri"
    //     await this.repo_lists.git_command_line(git_dir,cmd_str)
    //     process.exit(0)
    // }
